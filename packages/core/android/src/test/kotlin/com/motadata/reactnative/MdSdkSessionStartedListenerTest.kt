...
internal class MdSdkSessionStartedListenerTest {
    ...
    fun `set up`() {
        ...
        MdSdkSessionStartedListener.getInstance().setUiThreadExecutor(TestUiThreadExecutor())
    ...
    fun `𝕄 getInstance implements singleton correctly`() {
        // GIVEN
        val instance1 = MdSdkSessionStartedListener.getInstance()
        // WHEN
        val instance2 = MdSdkSessionStartedListener.getInstance()
        // THEN
    ...
    fun `𝕄 invalidate() invalidates previous instance`() {
        ...
        MdSdkSessionStartedListener.invalidate()
        // THEN
        assertThat(instance1).isNotEqualTo(MdSdkSessionStartedListener.getInstance())
    }
    ...
    fun `𝕄 onSessionStarted internally catches exception if catalyst instance throws`() {
        ...
        val instance = MdSdkSessionStartedListener.getInstance()
    ...
    fun `𝕄 session ID is NOT sent via bridge W { setReactContext fires before onRnSdkInitialized }`() { // ktlint-disable-line max-line-length
        ...
        // THEN — bridge must NOT be called (MotadataInternalReactBridge not yet registered)
        ...
        // THEN — catch-up delivery is now safe and matches __motadataOnMessageReceived(eventName, data)
