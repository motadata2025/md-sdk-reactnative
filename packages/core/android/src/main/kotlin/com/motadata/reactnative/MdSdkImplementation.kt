...
class MdSdkImplementation(
    ...
    fun initialize(configuration: ReadableMap, promise: Promise) {
        val ddSdkConfiguration = configuration.asMdSdkConfiguration()

        // On new arch MdSdk is a lazy TurboModule — it's instantiated on the first JS-side
        ...
        MdSdkSessionStartedListener.getInstance().setReactContext(reactContext)

        val nativeInitialization = MdSdkNativeInitialization(appContext, motadata, ddTelemetry)
        nativeInitialization.initialize(ddSdkConfiguration)

        val activity = reactContext.currentActivity
        if (ddSdkConfiguration.rumConfiguration != null && activity != null) {
            motadata.getRumMonitor()._getInternal()?.enableJankStatsTracking(activity)
        }
    ...
    fun addAttribute(key: String, value: ReadableMap, promise: Promise) {
        ...
        motadata.addRumGlobalAttribute(key, attributeValue)
    ...
    fun removeAttribute(key: String, promise: Promise) {
        motadata.removeRumGlobalAttribute(key)
    ...
    fun addAttributes(attributes: ReadableMap, promise: Promise) {
        motadata.addRumGlobalAttributes(attributes.toHashMap())
    ...
    fun removeAttributes(keys: ReadableArray, promise: Promise) {
        ...
        motadata.removeRumGlobalAttributes(keysStringArray)
    ...
    fun addUserExtraInfo(
        ...
        motadata.addUserExtraInfo(extraInfoMap)
        promise.resolve(null)
    }
    ...
    fun addAccountExtraInfo(
        ...
        motadata.addAccountExtraInfo(extraInfoMap)
        promise.resolve(null)
    }
    ...
    fun setTrackingConsent(trackingConsent: String, promise: Promise) {
        motadata.setTrackingConsent(buildTrackingConsent(trackingConsent))
        promise.resolve(null)
    }
    ...
    private fun configureSynthetics() {
        if (MdSdkSynthetics.testId.isNullOrBlank() || MdSdkSynthetics.resultId.isNullOrBlank()) {
            return
        }

        motadata.getRumMonitor()._getInternal()?.setSyntheticsAttribute(
            MdSdkSynthetics.testId,
            MdSdkSynthetics.resultId
        )
    }
    ...
    private fun createFrameRateProvider(
        ddSdkConfiguration: MdSdkConfiguration
    ): FrameRateProvider? {
    ...
    private fun buildFrameTimeCallback(
        ...
        return {
            ...
            if (jsLongTasksMonitoringEnabled &&
                ...
                motadata.getRumMonitor()._getInternal()?.addLongTask(it.toLong(), "javascript")
            }
        }
    }
    ...
    internal companion object {
        ...
        internal const val MD_VERSION = "_dd.version"
        internal const val MD_VERSION_SUFFIX = "_dd.version_suffix"
        internal const val MD_NEEDS_CLEAR_TEXT_HTTP = "_dd.needsClearTextHttp"
        internal const val MD_DROP_RESOURCE = "_dd.resource.drop_resource"
        internal const val MD_DROP_ACTION = "_dd.action.drop_action"
