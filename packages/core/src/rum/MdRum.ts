...
const RUM_MODULE = 'com.motadata.reactnative.rum';
...
class MdRumWrapper implements MdRumType {
    // eslint-disable-next-line global-require, @typescript-eslint/no-var-requires
    private nativeRum: MdNativeRumType = require('../specs/NativeMdRum')
        .default;
    ...
    addError = (
        ...
        updatedContext[MdAttributes.errorSourceType] = 'react-native';

        if (debugId) {
            updatedContext[MdAttributes.debugId] = debugId;
        }
    ...
    generateTraceId(): MotadataTracingIdentifier {
        return new MotadataTracingIdentifier(TracingIdentifier.createTraceId());
    }

    generateSpanId(): MotadataTracingIdentifier {
        return new MotadataTracingIdentifier(TracingIdentifier.createSpanId());
    }
    ...
    private getStopActionNativeCallArgs = (
        ...
        if (isOldStopActionAPI(args)) {
            ...
            InternalLog.log(
                'MdRum.startAction needs to be called before MdRum.stopAction',
                SdkVerbosity.WARN
            );
        } else {
            InternalLog.log(
                'MdRum.stopAction was called with wrong arguments',
                SdkVerbosity.WARN
            );
        }
...
export const MdRum = getGlobalInstance(RUM_MODULE, () => new MdRumWrapper());
