...
import { MotadataTracingContext } from '../instrumentation/resourceTracking/distributedTracing/MotadataTracingContext';
import { MotadataTracingIdentifier } from '../instrumentation/resourceTracking/distributedTracing/MotadataTracingIdentifier';
...
describe('MdRum', () => {
    ...
    describe('Context validation', () => {
        describe('MdRum.startView', () => {
            test('uses given context when context is valid', async () => {
                const context = {
                    testA: 123,
                    testB: 'ok'
                };
                await MdRum.startView('key', 'name', context);

                expect(NativeModules.MdRum.startView).toHaveBeenCalledWith(
                    expect.anything(),
                    expect.anything(),
                    context,
                    expect.anything()
        ...
        describe('MdRum.stopView', () => {
            test('uses given context when context is valid', async () => {
                const context = {
                    testA: 123,
                    testB: 'ok'
                };
                await MdRum.startView('key', 'name');
                await MdRum.stopView('key', context);

                expect(NativeModules.MdRum.stopView).toHaveBeenCalledWith(
                    'key',
                    context,
                    expect.anything()
                );
        ...
        describe('MdRum.startAction', () => {
            test('uses given context when context is valid', async () => {
                const context = {
                    testA: 123,
                    testB: 'ok'
                };
                await MdRum.startAction(RumActionType.SCROLL, 'name', context);

                expect(NativeModules.MdRum.startAction).toHaveBeenCalledWith(
                    expect.anything(),
                    expect.anything(),
                    context,
                    expect.anything()
        ...
        describe('MdRum.stopAction', () => {
            describe('New API', () => {
                test('uses given context when context is valid', async () => {
                    ...
                    await MdRum.startAction(RumActionType.SCROLL, 'name');
                    await MdRum.stopAction(
                        RumActionType.SCROLL,
                        'name',
                        context
                    );

                    expect(NativeModules.MdRum.stopAction).toHaveBeenCalledWith(
                        RumActionType.SCROLL,
                        'name',
                        context,
                        expect.anything()
            ...
            describe('Old API', () => {
                test('uses given context when context is valid', async () => {
                    ...
                    await MdRum.startAction(RumActionType.SCROLL, 'name');
                    await MdRum.stopAction(context);
                ...
                test('uses empty context with error when context is invalid or null', async () => {
                    await MdRum.startAction(RumActionType.SCROLL, 'name');
                    await MdRum.stopAction(undefined);
        ...
        describe('MdRum.startResource', () => {
            test('uses given context when context is valid', async () => {
                const context = {
                    testA: 123,
                    testB: 'ok'
                };
                await MdRum.startResource('key', 'method', 'url', context);

                expect(NativeModules.MdRum.startResource).toHaveBeenCalledWith(
                    'key',
                    'method',
                    'url',
                    context,
        ...
        describe('MdRum.stopResource', () => {
            test('uses given context when context is valid', async () => {
                ...
                await MdRum.startResource('key', 'method', 'url', {});
                await MdRum.stopResource('key', 200, 'other', -1, context);
        ...
        describe('Tracing Context APIs', () => {
            describe('Types and Enums', () => {
                ...
                it('exposes MotadataTracingIdentifier enum', () => {
                    expect(MotadataTracingIdentifier).toBeDefined();
                });

                it('exposes MotadataTracingContext class', () => {
                    expect(MotadataTracingContext).toBeDefined();
                });
            });

            describe('MdRum.generateTraceId', () => {
                it('generates 128-bit trace ID (100 iterations)', () => {
                    for (let i = 0; i < 100; i++) {
                        const traceId = MdRum.generateTraceId();
            ...
            describe('MdRum.generateSpanId', () => {
                it('generates 64-bit span ID (100 iterations)', () => {
                    for (let i = 0; i < 100; i++) {
                        const spanId = MdRum.generateSpanId();
            ...
            describe('MdRum.getTracingContext', () => {
                it('returns tracing context with MOTADATA propagator and sampling rate (50% 0, 50% 100)', () => {
                    for (let i = 0; i < 100; i++) {
                        ...
                        const tracingContext = MdRum.getTracingContext(
                            url,
                            tracingSamplingRate,
                            firstPartyHosts
                        );
                        ...
                        TracingContextUtils.verifyMotadataHeaders(
                            headers,
            ...
            describe('MdRum.getTracingContextForPropagators', () => {
                it('returns tracing context with MOTADATA propagator and sampling rate (50% 0, 50% 100)', () => {
                    for (let i = 0; i < 100; i++) {
                        const tracingSamplingRate =
                            Math.random() < 0.5 ? 0 : 100;

                        const tracingContext = MdRum.getTracingContextForPropagators(
                            [PropagatorType.MOTADATA],
                            tracingSamplingRate
                        );
        ...
        describe('MdRum.addTiming', () => {
            it('calls the native SDK when setting a timing', async () => {
                // GIVEN
                const timingName = 'testTiming';

                // WHEN
                await MdRum.addTiming(timingName);

                // THEN
                expect(NativeModules.MdRum.addTiming).toHaveBeenCalledTimes(1);
                expect(NativeModules.MdRum.addTiming).toHaveBeenCalledWith(
                    timingName
                );
            });
        });

        describe('MdRum.addViewAttribute', () => {
            it('calls the native SDK when setting a view attribute', async () => {
                ...
                await MdRum.addViewAttribute(key, value);

                // THEN
                expect(
                    NativeModules.MdRum.addViewAttribute
        ...
        describe('MdRum.removViewAttribute', () => {
            it('calls the native SDK when removing a view attribute', async () => {
                // GIVEN
                const key = 'testAttribute';

                // WHEN
                await MdRum.removeViewAttribute(key);

                // THEN
                expect(
                    NativeModules.MdRum.removeViewAttribute
                ).toHaveBeenCalledTimes(1);
                expect(
                    NativeModules.MdRum.removeViewAttribute
        ...
        describe('MdRum.addViewAttributes', () => {
            it('calls the native SDK when setting view attributes', async () => {
                ...
                // WHEN
                await MdRum.addViewAttributes(attributes);

                // THEN
                expect(
                    NativeModules.MdRum.addViewAttributes
                ).toHaveBeenCalledTimes(1);
                expect(
                    NativeModules.MdRum.addViewAttributes
        ...
        describe('MdRum.removViewAttributes', () => {
            it('calls the native SDK when removing view attributes', async () => {
                // GIVEN
                const keysToDelete = ['test1', 'test2'];

                // WHEN
                await MdRum.removeViewAttributes(keysToDelete);

                // THEN
                expect(
                    NativeModules.MdRum.removeViewAttributes
                ).toHaveBeenCalledTimes(1);
                expect(
                    NativeModules.MdRum.removeViewAttributes
        ...
        describe('MdRum.addAction', () => {
            test('uses given context when context is valid', async () => {
                const context = {
                    testA: 123,
                    testB: 'ok'
                };
                await MdRum.addAction(RumActionType.SCROLL, 'name', context);
    ...
    describe('MdRum.stopAction', () => {
        ...
        test('calls the native SDK when called with new API with default values', async () => {
            await MdRum.stopAction(RumActionType.SCROLL, 'page');
            expect(NativeModules.MdRum.stopAction).toHaveBeenCalledWith(
                RumActionType.SCROLL,
                'page',
                {},
        ...
        test('does not call the native SDK when startAction has not been called before and using old API', async () => {
            await MdRum.stopAction({ user: 'me' }, 789);
            expect(NativeModules.MdRum.stopAction).not.toHaveBeenCalled();
            expect(NativeMdSdk.telemetryDebug).not.toHaveBeenCalled();
        });

        test('calls the native SDK when called with old API', async () => {
            await MdRum.startAction(RumActionType.SCROLL, 'page_old_api');
            await MdRum.stopAction({ user: 'me' }, 789);
            expect(NativeModules.MdRum.stopAction).toHaveBeenCalledWith(
                RumActionType.SCROLL,
                'page_old_api',
            ...
            expect(NativeMdSdk.telemetryDebug).toHaveBeenCalledWith(
                'DMdRum.stopAction called with the old signature'
            );
        });
        ...
        test('cleans the action data when stopAction is called', async () => {
            ...
            await MdRum.stopAction();
            await MdRum.stopAction();
            expect(NativeModules.MdRum.stopAction).toHaveBeenCalledTimes(1);
        });
    });

    describe('MdRumWrapper', () => {
        beforeEach(() => {
            jest.clearAllMocks();
            BufferSingleton.onInitialization();
        });
        ...
        it('M add error source type W addError()', async () => {
            ...
            // When
            MdRum.addError(message, source, stacktrace);

            // Then
            expect(NativeModules.MdRum.addError.mock.calls.length).toBe(1);
            expect(NativeModules.MdRum.addError.mock.calls[0][0]).toBe(message);
            expect(NativeModules.MdRum.addError.mock.calls[0][1]).toBe(source);
            expect(NativeModules.MdRum.addError.mock.calls[0][2]).toBe(
                stacktrace
            );
            const context = NativeModules.MdRum.addError.mock.calls[0][3];
        ...
        it('M add error fingerprint W addError() { with custom fingerprint }', async () => {
            ...
            expect(NativeModules.MdRum.addError.mock.calls[0][5]).toBe(
                'custom-fingerprint'
            );
        });
        ...
        it('M add error source type W addError() {with custom attributes}', async () => {
            ...
            // When
            MdRum.addError(message, source, stacktrace, attributes);
    ...
    describe('MdRum.addError', () => {
        it('registers event mapper and maps error', async () => {
            ...
            await MdRum.addError('Old message', ErrorSource.CUSTOM, 'stack', {
                isFatal: false
            });
            expect(NativeModules.MdRum.addError).toHaveBeenCalledWith(
                'New message',
        ...
        it('drops the event if the mapper returns null', async () => {
            ...
            expect(NativeModules.MdRum.addError).not.toHaveBeenCalled();
        });
        ...
        it('can inject fingerprint from custom mapper', async () => {
            ...
            await MdRum.addError('ERROR MESSAGE', ErrorSource.CUSTOM, 'stack', {
                isFatal: true
            });
            expect(NativeModules.MdRum.addError).toHaveBeenCalledWith(
                'ERROR MESSAGE',
    ...
    describe('MdRum.stopResource', () => {
        it('registers event mapper and maps resource', async () => {
            ...
            await MdRum.stopResource('key', 200, 'xhr', 302, {}, 245);
            expect(NativeModules.MdRum.stopResource).toHaveBeenCalledWith(
                'key',
                200,
                'xhr',
    ...
    describe('MdRum.stopSession', () => {
        it('calls the native API', async () => {
            await MdRum.stopSession();
            expect(NativeModules.MdRum.stopSession).toHaveBeenCalledWith();
        });
    });

    describe('MdRum.getCurrentSessionId', () => {
        it('calls the native API if SDK is initialized', async () => {
            GlobalState.isInitialized = true;
            const sessionId = await MdRum.getCurrentSessionId();
            expect(NativeModules.MdRum.getCurrentSessionId).toHaveBeenCalled();
    ...
    describe('MdRum.getCurrentSessionId', () => {
        it('returns undefined if SDK is not initialized', async () => {
            ...
            const sessionId = await MdRum.getCurrentSessionId();
            expect(
                NativeModules.MdRum.getCurrentSessionId
            ).toHaveBeenCalledTimes(0);
            expect(sessionId).toBe(undefined);
        });
    });

    describe('MdRum.addViewLoadingTime', () => {
        it('calls the native API', async () => {
            await MdRum.addViewLoadingTime(true);
            await MdRum.addViewLoadingTime(false);

            expect(
                NativeModules.MdRum.addViewLoadingTime
    ...
    describe('PropagatorTypes', () => {
        it('matches with the native name of propagators', () => {
             ...
             */
            expect(PropagatorType.MOTADATA).toBe('motadata');
