...
const RUM_RESOURCE_TRACKING_MODULE =
    'com.motadata.reactnative.rum.resource_tracking';
...
class RumResourceTracking {
    ...
    startTracking({
        ...
        if (this._isTracking) {
            InternalLog.log(
                'Motadata SDK is already tracking XHR resources',
                SdkVerbosity.WARN
            );
            return;
        }
        ...
        InternalLog.log(
            'Motadata SDK is tracking XHR resources',
            SdkVerbosity.INFO
        );
     ...
     * (MotadataProvider.initialize) where tracking is started at provider
...
export const MdRumResourceTracking = getGlobalInstance(
