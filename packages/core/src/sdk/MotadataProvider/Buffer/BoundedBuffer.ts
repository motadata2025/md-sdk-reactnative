...
import { NativeMdSdk } from '../../MdSdkInternal';

import { MotadataBuffer } from './MotadataBuffer';
...
export class BoundedBuffer extends MotadataBuffer {
    ...
    drain = async () => {
        for (
            ...
            try {
                ...
                InternalLog.log(
                    `Error while draining Motadata Buffer: ${error}`,
                    SdkVerbosity.WARN
                );
            }
        }
    ...
    private logRandomIdGenerationError = (error: any): void => {
        ...
        console.warn(
            `[Motadata] Could not generate enough random numbers for RUM buffer. Please check that Math.random is not overwritten. Math.random returns: ${Math.random()}`
        );
    };
