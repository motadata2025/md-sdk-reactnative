...
afterEach(() => {
    MdRumErrorTracking['isTracking'] = false;
...
it('M intercept and send a RUM event W onGlobalError() {no message}', async () => {
    ...
    // WHEN
    MdRumErrorTracking.onGlobalError(error, is_fatal);
    await flushPromises();

    // THEN
    expect(MdRum.addError).toHaveBeenCalledTimes(1);
    expect(MdRum.addError).toHaveBeenCalledWith(
        'Unknown Error',
        'SOURCE',
...
it('M intercept and send a RUM event W onGlobalError() {Error object}', async () => {
    ...
    expect((MdRum.addError as any).mock.calls[0][2]).toContain(
        '/packages/core/src/__tests__/rum/instrumentation/MdRumErrorTracking.test.tsx'
    );
    expect(baseErrorHandlerCalled).toStrictEqual(true);
});
...
it('M intercept and send a RUM event W onConsole() {Error with source file info}', async () => {
    ...
    // WHEN
    MdRumErrorTracking.onConsoleError(message, error);
    await flushPromises();
...
it('M intercept and send a RUM event W onConsole() {message only}', async () => {
    ...
    // WHEN
    MdRumErrorTracking.onConsoleError(message);
    await flushPromises();
