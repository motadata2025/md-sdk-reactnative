...
import { MdRum, InternalLog } from '@motadata365/mobile-react-native';
...
} from '../../../rum/instrumentation/MdRumReactNavigationTracking';
import { MdRumReactNavigationTracking } from '../../../rum/instrumentation/MdRumReactNavigationTracking';
...
jest.mock('@motadata365/mobile-react-native', () => {
    return {
        MdRum: {
...
beforeEach(() => {
    ...
    mocked(MdRum.startView).mockClear();
    mocked(MdRum.stopView).mockClear();
    mocked(AppState.addEventListener).mockClear();
    mocked(BackHandler.exitApp).mockClear();

    // @ts-ignore
    MdRumReactNavigationTracking._resetInternalStateForTesting();
});
...
describe.each([
    ...
    'MdRumReactNavigationTracking on react-navigation v%s',
    (
        version,
        {
            FakeNavigator1,
        ...
        describe('startTrackingViews', () => {
            it('sends a related RUM ViewEvent when switching screens { navigationContainer listener attached }', async () => {
                ...
                MdRumReactNavigationTracking.startTrackingViews(
                ...
                // THEN
                expect(MdRum.startView).toHaveBeenCalledTimes(2);
                expect(MdRum.startView).toHaveBeenCalledWith(
                    expect.any(String),
                    'Home'
                );
                expect(MdRum.startView).toHaveBeenCalledWith(
            ...
            it('sends a related RUM ViewEvent when switching screens { viewPredicate returns null }', async () => {
                ...
                // THEN
                expect(MdRum.startView).not.toHaveBeenCalled();
            });
            ...
            it('sends a related RUM ViewEvent when switching screens { viewTrackingPredicate returns true for Home screen only }', async () => {
                ...
                // THEN
                expect(MdRum.startView).toHaveBeenCalledTimes(1);
                expect(MdRum.startView).toHaveBeenCalledWith(
                    expect.any(String),
                    'Home'
                );
                expect(MdRum.startView).not.toHaveBeenCalledWith(
                    expect.any(String),
                    'About'
                );
            });
            ...
            it('does nothing when startTrackingViews { undefined any }', async () => {
                // WHEN
                MdRumReactNavigationTracking.startTrackingViews(null);

                // THEN
                expect(MdRum.startView).toHaveBeenCalledTimes(0);
                expect(InternalLog.log).toHaveBeenCalledTimes(1);
                expect(InternalLog.log).toHaveBeenCalledWith(
                    MdRumReactNavigationTracking.NULL_NAVIGATION_REF_ERROR_MESSAGE,
                    'error'
                );
            });
            ...
            it('sends a RUM ViewEvent for each when startTrackingViews { multiple navigation containers when first not detached }', async () => {
                ...
                expect(MdRum.startView).toHaveBeenCalledTimes(2);
                expect(InternalLog.log).toHaveBeenCalledWith(
                    MdRumReactNavigationTracking.NAVIGATION_REF_IN_USE_ERROR_MESSAGE,
                    'error'
                );
            });
        ...
        describe('stopTrackingViews', () => {
            it('does nothing when switching screens { navigationContainer listener detached }', async () => {
                ...
                // WHEN
                MdRumReactNavigationTracking.stopTrackingViews(
                    navigationRef.current
                );
                expect(goToAboutButton).toBeTruthy();
                fireEvent(goToAboutButton, 'press');
            ...
            it('sends a RUM ViewEvent for each when startTrackingViews { multiple navigation containers when first is detached }', async () => {
                ...
                // THEN
                expect(MdRum.startView).toHaveBeenCalledTimes(4);
                expect(MdRum.startView).toHaveBeenCalledWith(
        ...
        describe.each([
            ...
            (reactNativeVersion, AppStateMockVersion) => {
                ...
                it('registers and unregisters AppState', async () => {
                    ...
                    expect(MdRum.stopView).toHaveBeenCalledTimes(1);
                });
                ...
                it('does not log AppState changes when tracking is stopped', async () => {
                    ...
                    // THEN
                    expect(MdRum.stopView).not.toHaveBeenCalled();
                ...
                it('stops active view when app goes into background', async () => {
                    ...
                    expect(MdRum.stopView).toHaveBeenCalledWith(transformedKey);
                ...
                it('does not crash when stopTrackingViews is called before startTrackingViews', async () => {
                    ...
                    // THEN
                    expect(MdRum.stopView).toHaveBeenCalled();
                });
        ...
        describe('Android back handler', () => {
            it('does not send an error when the app closes with Android back button', async () => {
                ...
                expect(MdRum.startView).toHaveBeenLastCalledWith(
                    expect.any(String),
                    'About'
                );
                expect(InternalLog.log).not.toHaveBeenCalledWith(
